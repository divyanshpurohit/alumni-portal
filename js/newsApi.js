    ...
    fetch(apiUrls.topThreeNews, {
        ...
        .then(data => {
            for (let i = 0; i < data.length; i++) {
                createNewsElements(data[i].publishedOn, data[i].title);
            }
        })
        .catch(err => {
            console.log(err);
...
function createNewsElements(d, title) {
    ...
    document.getElementById("news-div").appendChild(divElement);
}
